...
def encode_video(frames, method='JPEG', **kwargs):
    ...
    if method == 'JPEG':
        ...
        compressed_frames = []
        for frame in frames:
            compressed_frame = simplejpeg.encode_jpeg(frame, quality=quality, colorspace='RGB')
        ...
        crf = kwargs.get('h265_crf', 23) # Constant Rate Factor (0-51, lower is better quality)
        ...
        with suppress_av_output():
            ...
            stream.width = w
            stream.height = h
            stream.pix_fmt = 'yuv420p' # yuv420p is the only supported pixel format for H.265
