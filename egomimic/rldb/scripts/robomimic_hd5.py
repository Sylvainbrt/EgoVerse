...
class RobomimicHD5Extractor:
        ...
        for episode_key in episode_keys:
            ...
            for action_key in config["action_keys"]:
                if action_key not in episode:
                    raise ValueError(f"Missing action key: {action_key} in {episode_key}")
            ...
            for obs_key in config["obs_keys"]:
                if obs_key not in obs:
                    raise ValueError(f"Missing observation key: {obs_key} in {episode_key}")
            ...
            for obs_key in config["obs_keys"]:
                if obs_key in episode["obs"]:
                    if "img" or "image" in obs_key and image_compressed:
                        ...
                        if encode_as_video:
                            frame[f"obs.{obs_key}"] = torch.from_numpy(cv2.imdecode(image, 1).transpose(2, 0, 1))
                        else:
                            frame[f"obs.{obs_key}"] = torch.from_numpy(image)
                    else:
                        frame[f"obs.{obs_key}"] = torch.from_numpy(episode["obs"][obs_key][frame_idx])
            frames.append(frame)
        return frames
    ...
    def define_features(hdf5_file_path, config, image_compressed, encode_as_video):
        ...
        allowed_keys = set(["obs/" + key for key in config.get("obs_keys", [])] + config.get("action_keys", []))
...
class DatasetConverter:
    ...
    def __init__(
        ...
        self.logger.info("Initialized DatasetConverter with the following configuration:")
        ...
        self.features = RobomimicHD5Extractor.define_features(raw_path, self.config, self.image_compressed, self.encode_as_video)
    ...
    def extract_episode(self, episode_key, episode, task_description=''):
        ...
        frames = RobomimicHD5Extractor.extract_episode_frames(episode, self.config, self.image_compressed, self.encode_as_video)

        for frame in frames:
            self.dataset.add_frame(frame)
        
        self.logger.info(f"Saving Episode {episode_key} with Description: {task_description} ...")
    ...
    def extract_episodes(self, episode_description={}):
        ...
        with h5py.File(self.raw_path, "r") as data:
            for episode_key in self.episode_keys:
                episode = data["data"][episode_key]
                self.extract_episode(episode_key, episode, task_description=episode_description.get(episode_key, ''))
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Convert robomimic dataset to Lerobot format.")
    parser.add_argument("--name", type=str, required=True, help="Name to store dataset as")
    parser.add_argument("--raw-path", type=Path, required=True, help="Path to the raw HDF5 dataset.")
    parser.add_argument("--dataset-repo-id", type=str, required=True, help="Hugging Face repository ID.")
    parser.add_argument("--config-path", type=Path, required=True, help="Path to the JSON config file.")
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory for the processed dataset.")
    parser.add_argument("--fps", type=int, required=True, help="Frames per second for the dataset.")
    parser.add_argument("--image-compressed", action="store_true", help="Flag to indicate if images are compressed.")
    parser.add_argument("--encode-as-video", action="store_true", help="Flag to encode images as video.")
    ...
    parser.add_argument("--private", action="store_true", help="Make the dataset private on the Hugging Face Hub.")
    parser.add_argument("--license", type=str, default="apache-2.0", help="License for the dataset.")
    parser.add_argument("--overcap", action="store_true", help="Flag for using 'overcap' partition in SLURM.")
    parser.add_argument("--gpus-per-node", type=int, default=1, help="Number of GPUs per SLURM node.")
    parser.add_argument("--num-nodes", type=int, default=1, help="Number of SLURM nodes.")
    parser.add_argument("--partition", type=str, default="hoffman-lab", help="SLURM partition name.")
    parser.add_argument("--ignore_episode_keys", action="store_true", help="Use episode keys inside config")
...
def main(args):
    ...
    if args.push:
        converter.push_dataset_to_hub(
            dataset_tags=RobomimicHD5Extractor.TAGS, private=args.private, push_videos=True, license=args.license
        )
