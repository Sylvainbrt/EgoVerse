...
def main(model: str, interface: str, urdf_path: str):
    ...
    while True:
        ...
        gripper_vels.pop(0)
        gripper_acc = 1 / 2 * (gripper_vels[-1] - gripper_vels[0]) / (
            dt * (len(gripper_vels) - 1)
        ) + 1 / 2 * (gripper_vels[-2] - gripper_vels[2]) / (
            dt * (len(gripper_vels) - 3)
        )
        ...
        print(
            f"gripper pos: {joint_state.gripper_pos:+.3f}, vel: {joint_state.gripper_vel:+.3f}, acc: {gripper_acc:+.3f}, torque: {joint_state.gripper_torque:+.3f}"
        )
