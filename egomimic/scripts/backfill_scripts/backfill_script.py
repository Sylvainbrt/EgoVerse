...
def download_key(bucket: str, key: str, dest_path: str, s3_client, dry_run: bool = False):
    if dry_run:
...
def interactive_select(vrs_objects: List[Dict], key_map: Dict[str, Dict], bucket: str, prefix: str, s3_client, dry_run: bool = False, preview_chars: int = 1000) -> List[Dict]:
                ...
                if 1 <= idx <= max_idx:
                    ...
                    if json_meta:
                        try:
                            ...
                            print('\n--- JSON preview (first {n} chars) ---'.format(n=preview_chars))
                            print(textwrap.shorten(text, width=preview_chars, placeholder='...'))
                            print('--- end preview ---\n')
                        except ClientError as e:
...
def download_vrs_and_json(bucket: str, prefix: str, dest_dir: str, profile: str = None, dry_run: bool = False, interactive: bool = False, preview_chars: int = 1000):
    session_kwargs = {}
    if profile:
    ...
    if interactive:
        selected_objs = interactive_select(vrs_objects, key_map, bucket, prefix, s3_client, dry_run=dry_run, preview_chars=preview_chars)
    else:
    ...
    for obj in selected_objs:
        ...
        if json_key in key_set:
            ok_json = download_key(bucket, json_key, json_local, s3_client, dry_run=dry_run)
        else:
            try:
    ...
    logger.info("Finished. Processed %d .vrs files, downloaded %d (dry_run=%s)", total, downloaded, dry_run)


def parse_args(argv=None):
    p = argparse.ArgumentParser(description='Download .vrs and matching .json files from an S3 prefix')
    p.add_argument('--bucket', required=False, default=None, help='S3 bucket name (will prompt if missing)')
    p.add_argument('--prefix', required=False, default=None, help='S3 prefix to scan (e.g. "path/to/folder/") (will prompt if missing)')
    p.add_argument('--dest', required=False, default=None, help='Local destination directory (will prompt if missing)')
    p.add_argument('--profile', required=False, default=None, help='AWS profile name to use')
    p.add_argument('--dry-run', action='store_true', help='Do not download, only print what would be downloaded')
    p.add_argument('--interactive', action='store_true', help='Enter interactive mode to preview and select files to download')
    p.add_argument('--preview-chars', type=int, default=1000, help='Number of chars to show when previewing json metadata')
