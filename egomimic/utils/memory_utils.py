...
def print_epoch_memory_summary(epoch_stats_list):
    ...
    if len(epoch_stats_list) > 0:
        ...
        alloc_values = [s["cuda_alloc_gb"] for s in epoch_stats_list if s["cuda_alloc_gb"] > 0]
        reserved_values = [s["cuda_reserved_gb"] for s in epoch_stats_list if s["cuda_reserved_gb"] > 0]
        peak_values = [s["cuda_peak_gb"] for s in epoch_stats_list if s["cuda_peak_gb"] > 0]
        
        print("-" * 80)
        print(f"{'Min':<8} {min(rss_values):<12.2f} {min(alloc_values) if alloc_values else 0:<16.2f} {min(reserved_values) if reserved_values else 0:<18.2f} {min(peak_values) if peak_values else 0:<15.2f}")
        print(f"{'Max':<8} {max(rss_values):<12.2f} {max(alloc_values) if alloc_values else 0:<16.2f} {max(reserved_values) if reserved_values else 0:<18.2f} {max(peak_values) if peak_values else 0:<15.2f}")
        print(f"{'Avg':<8} {np.mean(rss_values):<12.2f} {np.mean(alloc_values) if alloc_values else 0:<16.2f} {np.mean(reserved_values) if reserved_values else 0:<18.2f} {np.mean(peak_values) if peak_values else 0:<15.2f}")
...
def print_parameter_gradient_status(model, epoch=None, print_all=False):
    ...
    try:
        if hasattr(model, 'named_parameters'):
            ...
            print(f"Model has attributes: {[attr for attr in dir(model) if not attr.startswith('_')]}")
            return
    ...
    if print_all and param_info:
        ...
        for info in param_info:
            print(f"{info['name']:<60} {str(info['requires_grad']):<15} {str(info['shape']):<30} {info['numel']:<15}")
